...
export const StepError: React.FC<StepErrorProps> = ({
  ...
  // Note: formatFieldName function removed - currently unused
  ...
  return (
    <div className={`step-error ${className}`} role="alert" aria-live="polite" aria-atomic="true">
          ...
          <ul className="step-error__field-list">
            ...
            {remainingErrors > 0 && (
              <li className="step-error__field-item step-error__field-item--more">
                {t('form.validation.required')} ({remainingErrors}{' '}
                {remainingErrors === 1 ? 'more error' : 'more errors'})
              </li>
            )}
          </ul>
        </div>
...
// Helper function to get field label from field name (currently unused, commented out)
