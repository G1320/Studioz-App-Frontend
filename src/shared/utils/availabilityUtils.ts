...
import { Item, Studio, Reservation } from 'src/types/index';
 ...
 */
export function isStudioOpenOnDay(
  date: Dayjs,
 ...
 */
export function meetsAdvanceBookingRequirement(
  date: Dayjs,
  item: Item
): boolean {
  if (!item.advanceBookingRequired?.value) {
 ...
 */
export function generateHoursFromTimeRanges(
 ...
 */
export function getPreparationTimeBuffer(
  bookedSlots: string[],
  prepTime?: Duration
): string[] {
 ...
 */
export function getTimeSlotsWithMetadata(
  date: Dayjs,
  context: AvailabilityContext
): TimeSlotResult[] {
  const { item } = context;
 ...
 */
export function getMaximumHours(
  startSlot: string,
  date: Dayjs,
  context: AvailabilityContext
): number {
 ...
 */
export function createShouldDisableTime(
