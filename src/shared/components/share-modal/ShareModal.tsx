...
export const ShareModal: React.FC<ShareModalProps> = ({ open, onClose, url, title }) => {
  ...
  return (
    <GenericModal open={open} onClose={onClose} className="share-modal">
      <div className="share-modal__content">
        <h2 className="share-modal__title">{t('share.title', 'Share listing')}</h2>
        ...
        <div className="share-modal__options">
          {shareOptions.map((option) =>
            option.onClick ? (
              <button
                key={option.id}
                className={`share-modal__option ${option.highlighted ? 'share-modal__option--highlighted' : ''} ${copied && option.id === 'copy' ? 'share-modal__option--success' : ''}`}
