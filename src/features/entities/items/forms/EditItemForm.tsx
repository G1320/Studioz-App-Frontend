...
export const EditItemForm = () => {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    // Update item first, then batch create add-ons
    updateItemMutation.mutate(formData as Item, {
      onSuccess: async () => {
        // Batch create add-ons if any are pending
        if (pendingAddOns.length > 0 && itemId) {
          try {
            await createAddOnsBatch(itemId, pendingAddOns);
            toast.success(`Item updated and ${pendingAddOns.length} add-on(s) created successfully`);
