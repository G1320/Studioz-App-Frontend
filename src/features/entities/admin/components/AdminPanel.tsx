...
const EmailTemplatesView = () => {
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      await sendTestEmail('admin@studioz.online', selectedType, previewMode);
      setTestEmailStatus({ type: 'success', message: '\u05E0\u05E9\u05DC\u05D7 \u05D1\u05D4\u05E6\u05DC\u05D7\u05D4!' });
      setTimeout(() => setTestEmailStatus(null), 5000);
    } catch (err) {
      console.error('Failed to send test email:', err);
      setTestEmailStatus({ type: 'error', message: '\u05E9\u05D2\u05D9\u05D0\u05D4 \u05D1\u05E9\u05DC\u05D9\u05D7\u05EA \u05D4\u05DE\u05D9\u05D9\u05DC' });
      setTimeout(() => setTestEmailStatus(null), 5000);
    } finally {
  ...
  return (
    <div className="admin-email-templates">
      <div className="admin-email-templates__sidebar">
        ...
        {/* Device Preview */}
        <div className="admin-email-templates__controls">
          <h3 className="admin-email-templates__controls-title">{'\u05EA\u05E6\u05D5\u05D2\u05EA \u05DE\u05DB\u05E9\u05D9\u05E8'}</h3>
          <div className="admin-email-templates__mode-toggle">
            <button
              className={cn('admin-email-templates__mode-btn', previewDevice === 'desktop' && 'admin-email-templates__mode-btn--active')}
            ...
            <button
              className={cn('admin-email-templates__mode-btn', previewDevice === 'mobile' && 'admin-email-templates__mode-btn--active')}
        ...
        <div className="admin-email-templates__controls">
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
              <span>{'\u05E1\u05D5\u05D2:'}</span>
              <span style={{ fontFamily: 'monospace', color: 'var(--color-brand)', fontSize: '11px' }}>
                {selectedType}
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>{'\u05E7\u05D8\u05D2\u05D5\u05E8\u05D9\u05D4:'}</span>
              <span>{EMAIL_TYPE_INFO[selectedType].category}</span>
            </div>
          </div>
        </div>
      ...
      <div className="admin-email-templates__preview">
        ...
        <div
          className="admin-email-templates__preview-content"
          style={{
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'flex-start',
            padding: '24px',
            background: previewMode === 'dark' ? '#1a1a2e' : '#f0f0f0',
            minHeight: '600px'
          }}
        >
          <iframe
