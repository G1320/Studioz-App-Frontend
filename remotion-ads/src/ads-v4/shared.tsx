/**
 * Shared Design System for Studioz B2B Ads V4
 * V4: No emojis — Lucide icons only. Only REAL features. Accurate stats.
 */
...
export const GoldButton: React.FC<{
  ...
  return (
    ...
    >
      <span
        style={{ fontFamily: FONT_HEADING, fontSize: s.fontSize, fontWeight: 700, color: DARK_BG }}
      >
        {text}
      </span>
      {arrow && <span style={{ fontSize: s.fontSize * 0.8, color: DARK_BG }}>←</span>}
    </div>
  );
};
...
export const Footer: React.FC<{ delay?: number }> = ({ delay = 0 }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, opacity: enter }}>
      ...
      <span
        style={{ fontFamily: FONT_HEADING, fontSize: 15, color: SUBTLE_TEXT, fontWeight: 500, letterSpacing: "0.5px" }}
      >
        studioz.co.il
      </span>
    </div>
...
export const FeatureCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div>
        <div
          style={{ fontFamily: FONT_HEADING, fontSize: 22, fontWeight: 700, color: LIGHT_TEXT, marginBottom: desc ? 4 : 0 }}
        >
          {title}
        </div>
        {desc && (
          <div style={{ fontFamily: FONT_BODY, fontSize: 15, color: SUBTLE_TEXT }}>{desc}</div>
        )}
      </div>
    </div>
  );
...
export const PainCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <Icon size={24} color={RED} strokeWidth={1.8} />
      </div>
      <div style={{ position: "relative" }}>
        <span style={{ fontFamily: FONT_BODY, fontSize: 24, color: SUBTLE_TEXT }}>{text}</span>
        {strikeDelay != null && (
          <div
            style={{
              position: "absolute",
...
/* ─── Screenshot Frame (no cropping) ─── */
export const ScreenshotFrame: React.FC<{
  src: string;
  delay?: number;
  borderRadius?: number;
}> = ({ src, delay = 0, borderRadius = 20 }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
/* ─── Phone Mockup (shows full image, no cropping) ─── */
export const PhoneMockup: React.FC<{
  src: string;
  delay?: number;
  width?: number;
}> = ({ src, delay = 0, width = 420 }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const slideUp = spring({ frame: frame - delay, fps, config: { damping: 14, stiffness: 60 } });
...
export const StatCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: FONT_BODY, fontSize: 15, color: SUBTLE_TEXT }}>{label}</div>
    </div>
  );
};
...
export const Badge: React.FC<{
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: FONT_HEADING, fontSize: 20, fontWeight: 600, color }}>{text}</span>
    </div>
  );
};
      ...
      {items.map((text, i) => {
        ...
        return (
          ...
          >
            ...
            />
            <span style={{ fontFamily: FONT_BODY, fontSize: 22, color: LIGHT_TEXT }}>{text}</span>
          </div>
        );
      })}
    </div>
...
export const StepCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontFamily: FONT_HEADING, fontSize: 26, fontWeight: 800, color: isLast ? DARK_BG : GOLD }}
        >
          {num}
        </span>
      </div>
      <div>
        <h3 style={{ fontFamily: FONT_HEADING, fontSize: 28, fontWeight: 700, color: LIGHT_TEXT, margin: "0 0 6px" }}>
          {title}
        </h3>
        <p
          style={{
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: DARK_BG, ...RTL }}>
      ...
      >
        ...
        >
          <Img src={staticFile("logo.png")} style={{ width: 90, height: 90, borderRadius: 20, marginBottom: 30 }} />
          <h2
            style={{
              fontFamily: FONT_HEADING,
              fontSize: 48,
          ...
          >
            <span style={{ fontFamily: FONT_HEADING, fontSize: 22, fontWeight: 600, color: SUCCESS }}>
              {badgeText}
            </span>
          </div>
          <p
          ...
          >
            <span style={{ fontFamily: FONT_HEADING, fontSize: 26, fontWeight: 700, color: DARK_BG }}>
              {buttonText}
            </span>
          </div>
          <span
...
export const ScreenshotCTAScene: React.FC<{
  ...
  headline: React.ReactNode;
  buttonText?: string;
}> = ({ screenshotSrc, headline, buttonText = "פרסם את האולפן שלך" }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: DARK_BG, ...RTL }}>
      ...
      >
        ...
        >
          <Img src={staticFile(screenshotSrc)} style={{ width: "100%", height: "auto" }} />
        </div>
      </div>
      <div
        style={{
