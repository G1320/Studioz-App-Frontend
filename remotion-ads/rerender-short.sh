...
echo ""
echo "════════════════════════════════════════"
echo "  Re-rendering 4:5 + 1:1 (smaller screenshots)"
echo "════════════════════════════════════════"
echo ""

# Clear old renders for both ratios
for folder in "1-PAIN" "2-OUTCOME" "3-FEATURES" "4-AUTHORITY" "5-CTA"; do
  rm -f "$BASE/$folder/4x5/"*.mp4 2>/dev/null || true
  rm -f "$BASE/$folder/1x1/"*.mp4 2>/dev/null || true
done
echo "Cleared old 4:5 and 1:1 renders."
echo ""

for ratio in 4x5 1x1; do
  echo ""
  echo "──── Rendering $ratio ────"
  echo ""
